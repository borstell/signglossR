...
#' @export
censor_image <- function(file, automatic=TRUE, style='blur', blur=8, region='100x150+100+100') {
  ...
  new_filename <- paste0("./", paste0(gsub(paste0(".", extension), "", gsub(".*/", "", file)), tag, ".", extension))
  if (automatic == FALSE) {
    if (style == "blur") {
    ...
    if (style == "blur") {
      ...
      system(paste0("convert -size ", width, "x", height, " xc:black black_mask_bg.jpg"))
      masked <- opencv::ocv_copyto(opencv::ocv_read("./black_mask_bg.jpg"), img, mask)
      opencv::ocv_write(masked, new_filename)
      system("rm black_mask_bg.jpg")
    }
  }
  return(new_filename)
}
