...
get_image_asl <- function(id, glosstext=FALSE, gravity="north", location="+10+20", fontsize=70) {
  ...
  if (glosstext == TRUE) {
    ...
    text_image <- magick::image_annotate(magick::image_read(img_name), as.character(signglossR::id2gloss_asl(id)), size = 70, font = "Helvetica", color = "white", weight = 700, gravity = gravity, location = location)
