...
get_image_asl <- function(id, destination="./", glosstext=FALSE, gravity="north", location="+20+20", fontsize=70) {
  ...
  if (glosstext == TRUE) {
    ...
    text_image <- magick::image_annotate(magick::image_read(img_name), as.character(signglossR::id2gloss_asl(id)), size = fontsize, color = "white", weight = fontsize*10, gravity = gravity, location = location)
