...
plot_openpose <- function(data, point_size=5, hands="both", right_color="green3", left_color="red3", path_color=FALSE, silhouette=0) {
  ...
  plot <- ggplot2::ggplot() +
    ...
    # Torso
    ggforce::geom_ellipse(data=subset(data, data$frame==1), ggplot2::aes(x0 = mean(data$x_0), y0 = mean(data$y_1)+abs(mean(data$y_0)-mean(data$y_8))*.3, a = coord_dist(mean(data$x_2), mean(data$y_2), mean(data$x_1), mean(data$y_1)), b =  coord_dist(mean(data$x_1), mean(data$y_1), mean(data$x_8), mean(data$y_8))*.4, angle=0), fill = silhouette_color, alpha=silhouette_alpha, linetype=0) +
    # Left + Right shoulder
    ...
    ggplot2::geom_segment(data=subset(data, data$frame==1), ggplot2::aes(x = mean(data$x_2), y = mean(data$y_2), xend = mean(data$x_3), yend = mean(data$y_3)), color = silhouette_color, alpha=silhouette_alpha, size=13, linejoin = "round") +
    ggplot2::geom_segment(data=subset(data, data$frame==1), ggplot2::aes(x = mean(data$x_3), y = mean(data$y_3), xend = mean(data$x_3), yend = mean(data$y_3)+coord_dist(mean(data$x_2), mean(data$y_2), mean(data$x_3), mean(data$y_3))*.6), color = silhouette_color, alpha=silhouette_alpha, size=11, linejoin = "round") +
    # Left arm (upper, lower)
    ggplot2::geom_segment(data=subset(data, data$frame==1), ggplot2::aes(x = mean(data$x_5), y = mean(data$y_5), xend = mean(data$x_6), yend = mean(data$y_6)), color = silhouette_color, alpha=silhouette_alpha, size=13, linejoin = "round") +
    ggplot2::geom_segment(data=subset(data, data$frame==1), ggplot2::aes(x = mean(data$x_6), y = mean(data$y_6), xend = mean(data$x_6), yend = mean(data$y_6)+coord_dist(mean(data$x_5), mean(data$y_5), mean(data$x_6), mean(data$y_6))*.6), color = silhouette_color, alpha=silhouette_alpha, size=11, linejoin = "round") +
    # Hands (right, left)
